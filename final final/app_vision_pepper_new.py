...
class Vision(object):
	...
	def gaze_detect(self, avertgaze, backgrnd):

		if backgrnd:	
			self.open = True
			self.avertgaze = avertgaze
			self.gazeController = GazeController(self.avertgaze, self.pip, self.pport)
			print " Starting the thread..."
			self.gazeController.stop_thread = False
			self.tmp = Thread(target = self.gazeController.run)
			self.tmp.start()
		elif self.open and not backgrnd:
	...
	def thread_gesture(self):
		gesture = Thread(target = self.gesture)
		gesture.start()
