...
class Dialogue(object):
    def __init__(self, pip, pport,jsonFilename, session):
	...
	str = "Hi i'm Pepper ! Welcome to the Emotion Game !"
	...
	if mode == 'a':
		self.vision.gaze_detect(False, True)
	if mode == 'b':
		self.vision.gaze_detect(True, True)
    ...
    def say(self, text):
	text = str(text)
	say(text, self.pip, self.pport)
                 
    def instructions(self, mode):
    ...
    def quiz(self, number, mode, question, emotion): 
	self.current_emotion = emotion
	string = "Question number {}".format(int(number))   
        say(string, self.pip, self.pport)
	if mode == 'b':	
		self.vision.gaze_detect(True, False)
		self.emotion_control.playEmotion(emotion)
		say(str(question), self.pip, self.pport)
		time.sleep(15)
		string = "This sentence is : 1 Happy, 2 Sad, 3 Angry, 4 Fear"
		say(string, self.pip, self.pport)
		self.vision.gaze_detect(True, True)
	elif mode == 'a':
		self.vision.gaze_detect(False, False)
		self.emotion_control.playEmotion(emotion)
                say(str(question), self.pip, self.pport)
		self.emotion_control.reset()
		time.sleep(15)
    ...
    def solution(self, mode, answer):
	if str(mode) == 'a':
		print(answer)
		prediction, score = predict_emotion(str(answer), self.current_emotion)
    ...
    def get_score(self, mode, score):
	if mode == 'a':
		string = "Your score is: {:.2f}".format(score)
		say(string, self.pip, self.pport)
	...
	if mode == 'b':
		if score == 1:
		    ...
		    string = "The answer seems to be uncorrect "
                    ...
                    say(string, self.pip, self.pport)
		    time.sleep(4)
                    string = "The correct answer is: {}".format(self.current_emotion)
		    say(string, self.pip, self.pport)
		    self.emotion_control.reset()
		    time.sleep(15)
