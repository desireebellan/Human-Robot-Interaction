...
def say(text, pip, pport):
    ...
    ttsProxy.setParameter("speed", 20)
...
class PlayEmotionController(object):
	...
	def reset(self, joint_values = None):
		...
		motionProxy = ALProxy("ALMotion", self.pip, self.pport)
                ...
                motionProxy.angleInterpolation(jointsNames, jointValues, jointTimes, isAbsolute)
	...
	def playEmotion(self, emotion):
		if emotion == "reset":
			self.reset()
		elif emotion == "happy" or emotion == "happiness":
...
if __name__ == "__main__":
	...
	parser.add_argument("--emotion", type = str, default = "", help="Naoqi port number")
	...
	emotionController = PlayEmotionController(pip, pport)
